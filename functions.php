...
if (isset($_POST['guess'])) {
    // echo "It worked finally! Id is " . $_POST['guess'];
    dealWithGuess();
}
...
function getCategory()
{
    ...
    else $category = $_SESSION['category'];
    return $category;
}
...
function getRating()
{
    ...
    if ($totalGuesses > 0) {
        for ($i = 0; $i < 5; $i++) {
            if ($correctGuesses / $totalGuesses >= $grade[$i][0]) return $grade = [$grade[$i][1], $grade[$i][2]];
        }
    } else return $grade = ["N/A", "secondary"];
}
...
function setUpGame()
{
    ...
    switch ($category) {
        ...
        case "lhb":
            $words = file("lhb.txt");
            setCategoryIcon("fa-user-astronaut");
            break;
        default:
            $words = file("fruits.txt");
            setCategoryIcon("fa-apple-alt");
    ...
    if (!isset($_SESSION['word'])) {
        ...
        if (!isset($_SESSION['correctGuesses'])) $_SESSION['correctGuesses'] = 0;
        if (!isset($_SESSION['incorrectGuesses'])) $_SESSION['incorrectGuesses'] = 0;
    }
}
...
function dealWithGuess()
{
    if (isset($_POST['guess'])) {
        if (!in_array($_POST['guess'], $_SESSION['guesses'])) {
            ...
            // echo "You have already guessed this letter.<br /><br />";
